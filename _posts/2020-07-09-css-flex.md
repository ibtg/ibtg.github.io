...
- `justify-items`가 `main-axis`에서 아이템들을 배치한다면, `align-items`는 `cross-axis`에서 어떻게 아이템들을 배치할 지 정한다

- basline

  - 다음과 같이 첫번째 `item`의 크기가 바뀌게 되었을 때 `item` 하나의 크기가 달라도 텍스트의 위치를 균일하게 맞추어서 정렬하도록 한다.
...
- `align-content`는 `justify-content`와 유사한데, `justify-content`가 `main-axis`의 `item`을 정렬한다면, `align-content`은 `cross-axis`의 `item`을 정렬한다
